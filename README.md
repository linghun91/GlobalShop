...
## 项目状态
当前项目已实现基础架构和主要功能，包括完整的拍卖系统、搜索功能、多语言支持和广播系统。最近完成了管理员查询玩家交易信息的功能和MySQL数据库支持。下一步计划优化界面美观度、提升性能并添加更多搜索和筛选选项。
...
- `GuiManager.java` - GUI管理器，负责创建和管理各种界面
  ...
  - 管理员查询玩家信息界面 (openPlayerInfoMenu)
  - 管理员查询玩家销售历史界面 (openPlayerSalesHistoryMenu)
  - 管理员查询玩家购买历史界面 (openPlayerPurchaseHistoryMenu)
  - 界面数据管理
    - 玩家页码跟踪 (playerPages)
    - 玩家搜索查询跟踪 (playerSearchQueries)
    - 自定义 InventoryHolder (包括管理员查询功能的新 Holder)
...
- `GuiListener.java` - GUI事件监听器，处理界面交互
  ...
  - 界面交互处理
    ...
    - handleAdminPlayerInfoClick - 处理管理员查询玩家信息界面点击
    - handleAdminHistoryClick - 处理管理员查询历史记录界面点击
...
- `AuctionCommand.java` - 拍卖命令处理类
  ...
  - 子命令处理
    ...
    - handleInfoCommand - 处理管理员查询玩家信息命令
...
- `AuctionItem.java` - 拍卖物品模型类
  - 属性
    ...
    - 价格信息（起拍价、一口价、当前价格）
    - 时间信息（开始时间、结束时间、售出时间）
    - 状态信息（活跃、已售出、已过期、邮箱待领）
    - 竞价信息（当前出价者UUID、出价者名称）
  - 方法
    - 获取和设置属性的方法
    - 状态判断方法（isActive, isExpired, canBuyNow, canBid等）
    - 时间格式化方法（getFormattedRemainingTime, getFormattedListTime, getFormattedSoldTime）
...
/gs info <玩家名> - 查询玩家拍卖行交易信息（需要管理员权限 globalshop.admin.info）
...
globalshop.admin.info - 管理员查询玩家信息权限
```
...
## 最近更新
1. MySQL数据库支持
   - 添加了对MySQL数据库的完整支持，同时保留原有SQLite功能
   - 在config.yml中添加数据库类型选择和MySQL连接配置选项
   - 重构DatabaseManager类，支持根据配置自动选择数据库连接方式
   - 优化SQL语句处理，确保在不同数据库类型下的兼容性
   - 添加MySQL错误处理和调试输出，提高系统稳定性
   - 支持无缝切换数据库类型，无需更改现有数据结构
...
21. 管理员查询玩家交易信息功能
    - 新增命令 `/gs info <玩家名>`，允许拥有 `globalshop.admin.info` 权限的管理员查询指定玩家的交易数据。
    - 添加了新的GUI界面：
        - 玩家信息主界面：显示玩家总售出/购买数量、总收入/支出（区分货币类型）。
        - 玩家销售历史界面：分页显示该玩家出售的所有物品记录。
        - 玩家购买历史界面：分页显示该玩家购买的所有物品记录。
    - 在 `DatabaseManager` 中添加了新的数据库查询方法，用于获取玩家的销售/购买历史和统计数据。
    - 在 `GuiManager` 中添加了新的 `InventoryHolder` 接口和打开、管理新界面的方法。
    - 在 `GuiListener` 中添加了处理新界面点击事件的逻辑。
    - 在 `MessageManager` 和 `message.yml` 中添加了所有新界面所需的文本配置。
